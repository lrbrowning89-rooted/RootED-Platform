...
import sqlite3, time, csv, io, uuid, os, logging.handlers, traceback
...
# Microsoft SSO (OIDC via common tenant)
oauth.register(
    name="microsoft",
...
def handle_exception(e):
    # Don't swallow debug mode errors – let Flask show full traceback
    if app.debug:
        raise e
...
def error_list():
    ...
    return render_template_string(
        html,
        rows=rows,
        datetime_format=datetime_format_filter,  # passed as a normal function
    )
