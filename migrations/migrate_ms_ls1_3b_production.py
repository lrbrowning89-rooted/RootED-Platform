...
JSON_PATH = ROOT / "data" / "imports" / "MS-LS1-3B.json"
NOW = int(time.time())
...
assert pack["objective_id"] == "MS-LS1-3B"
...
with sqlite3.connect(DB) as conn:
    ...
    for q in existing:
        ...
        """, (
            ...
            q["choice_d"],
            q["answer_key"],
            "Archived existing MS-LS1-3B question before production import",
            NOW,
        ))

    conn.execute("DELETE FROM questions WHERE objective_id = 'MS-LS1-3B'")
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?)
    """, (
        "ms_ls1_3b_production_import",
        f"Archived {len(existing)} existing MS-LS1-3B questions and imported {len(questions)} production questions.",
        NOW,
    ))

    conn.commit()

print(f"Archived {len(existing)} existing MS-LS1-3B questions")
print(f"Imported {len(questions)} production MS-LS1-3B questions")
print("MS-LS1-3B production import complete")
