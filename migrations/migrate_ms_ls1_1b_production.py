...
JSON_PATH = ROOT / "data" / "imports" / "MS-LS1-1B.json"
NOW = int(time.time())
...
assert pack["objective_id"] == "MS-LS1-1B"
assert pack["total_questions"] == 21
...
with sqlite3.connect(DB) as conn:
    ...
    for q in legacy_questions:
        ...
        """, (
            ...
            q["choice_d"],
            q["answer_key"],
            "Archived legacy MS-LS1-1B question before production import",
            NOW,
        ))

    conn.execute("DELETE FROM questions WHERE objective_id = 'MS-LS1-1B'")
    ...
    for q in questions:
        ...
        """, (
            ...
            q["question_type"],
            q["explanation"],
            json.dumps(q["identifier_tags"]),
            json.dumps(q["remediation_tags"]),
            q["model_id"],
            "MS-LS1-1B.json",
            NOW,
        ))
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?)
    """, (
        "ms_ls1_1b_production_import",
        f"Archived {len(legacy_questions)} legacy MS-LS1-1B questions and imported {len(questions)} production-approved questions.",
        NOW,
    ))
...
print("MS-LS1-1B production migration complete")
