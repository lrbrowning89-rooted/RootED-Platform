...
JSON_PATH = ROOT / "data" / "imports" / "MS-LS1-2B.json"
NOW = int(time.time())
...
assert pack["objective_id"] == "MS-LS1-2B"
...
with sqlite3.connect(DB) as conn:
    ...
    for q in existing:
        ...
        """, (
            ...
            q["answer_key"],
            "Archived existing MS-LS1-2B question before production import",
            NOW,
        ))

    conn.execute("DELETE FROM questions WHERE objective_id = 'MS-LS1-2B'")
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?)
    """, (
        "ms_ls1_2b_production_import",
        f"Archived {len(existing)} existing MS-LS1-2B questions and imported {len(questions)} production questions.",
        NOW,
    ))

    conn.commit()

print(f"Archived {len(existing)} existing MS-LS1-2B questions")
print(f"Imported {len(questions)} production MS-LS1-2B questions")
print("MS-LS1-2B production import complete")
