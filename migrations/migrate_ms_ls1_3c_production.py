...
JSON_PATH = ROOT / "data" / "imports" / "MS-LS1-3C.json"
NOW = int(time.time())
...
if isinstance(pack, list):
    ...
    assert all(q["standard_id"] == "MS-LS1-3" for q in questions)
    assert all(q["objective_id"] == "MS-LS1-3C" for q in questions)
else:
    assert pack["standard_id"] == "MS-LS1-3"
    assert pack["objective_id"] == "MS-LS1-3C"
...
with sqlite3.connect(DB) as conn:
    ...
    for q in existing:
        ...
        """, (
            ...
            q["choice_d"],
            q["answer_key"],
            "Archived existing MS-LS1-3C question before production import",
            NOW,
        ))

    conn.execute("DELETE FROM questions WHERE objective_id='MS-LS1-3C'")
...
for q in questions:
    ...
    if correct_answer in ("A", "B", "C", "D"):
        ...
        answer_key = None
        for letter, text in zip(("A", "B", "C", "D"), (choice_a, choice_b, choice_c, choice_d)):
            if text == correct_answer:
                answer_key = letter
                break

        assert answer_key is not None, f"Could not match correct_answer for {q['question_id']}"
    ...
    conn.execute("""
        INSERT OR REPLACE INTO question_metadata
          (question_id, difficulty, question_type, explanation,
           identifier_tags, remediation_tags, model_id,
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?)
    """, (
        "ms_ls1_3c_production_import",
        f"Archived {len(existing)} existing MS-LS1-3C questions and imported {len(questions)} production questions.",
        NOW,
    ))

    conn.commit()

print(f"Archived {len(existing)} existing MS-LS1-3C questions")
print(f"Imported {len(questions)} production MS-LS1-3C questions")
print("MS-LS1-3C production import complete")
