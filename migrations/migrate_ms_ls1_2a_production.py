...
JSON_PATH = ROOT / "data" / "imports" / "MS-LS1-2A.json"
NOW = int(time.time())

with open(JSON_PATH, "r", encoding="utf-8") as f:
    pack = json.load(f)

assert pack["standard_id"] == "MS-LS1-2"
assert pack["objective_id"] == "MS-LS1-2A"
...
with sqlite3.connect(DB) as conn:
    ...
    for q in existing:
        ...
        """, (
            ...
            q["answer_key"],
            "Archived existing MS-LS1-2A question before production import",
            NOW,
        ))

    conn.execute("DELETE FROM questions WHERE objective_id = 'MS-LS1-2A'")
    ...
    for q in questions:
        ...
        conn.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            q["question_id"], q["objective_id"], q["prompt"],
    ...
    conn.execute("""
        ...
        VALUES (?, ?, ?)
    """, (
        "ms_ls1_2a_production_import",
        f"Archived {len(existing)} existing MS-LS1-2A questions and imported {len(questions)} production questions.",
        NOW,
    ))

    conn.commit()

print(f"Archived {len(existing)} existing MS-LS1-2A questions")
print(f"Imported {len(questions)} production MS-LS1-2A questions")
print("MS-LS1-2A production import complete")